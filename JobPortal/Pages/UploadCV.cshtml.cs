...
namespace JobPortal.Pages
{
    public class UploadCVModel : PageModel
    {
        ...
        public UploadCVModel(JobSeekerDbContext cd)
        {
        ...
        public async Task<IActionResult> OnPostAsync(IFormFile file)
        {
            ...
            var path = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot/CV", tempUser.ID.ToString() + file.FileName);

            using (var stream = new FileStream(path, FileMode.Create))
            {
                await file.CopyToAsync(stream);
            }
            tempUser.ResumeLink = "CV/" + tempUser.ID.ToString() + file.FileName;
